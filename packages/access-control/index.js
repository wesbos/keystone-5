...
module.exports = {
  ...
  parseFieldAccess({ listKey, fieldKey, defaultAccess, access = defaultAccess }) {
    ...
    return parseAccess({
      ...
      defaultAccess,
      onGranularParseError: () => {
        throw new Error(`Must specify one of ${JSON.stringify(accessTypes)} access configs, but got ${JSON.stringify(Object.keys(access))}. (Did you mean to specify a declarative access control config? This can be done on lists only)`);
      },
      validateGranularType: (type, accessType) => {
        if (!['Boolean', 'Function'].includes(type)) {
