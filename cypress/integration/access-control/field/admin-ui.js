...
describe('Access Control Fields > Admin UI', () => {
  ...
  describe('creating', () => {
    describe('static', () => {
      ...
      accessCombinations.filter(({ create, read }) => !create && read).forEach(access => {
        it.skip(`does not show fields when not creatable: ${JSON.stringify(access)}`, () => {
          const name = getStaticListName(access);
          const queryName = `all${name}s`;
          const slug = listSlug(name);
    ...
    describe('dynamic', () => {
      ...
      // non-readable lists show access denied above
      accessCombinations.filter(({ create, read }) => create && read).forEach(access => {
        it(`shows create option when at least one field creatable (list view): ${JSON.stringify(access)}`, () => {
        ...
        it(`shows create option when at least one field creatable (item view): ${JSON.stringify(access)}`, () => {
      ...
      // non-readable lists show access denied above
      accessCombinations.filter(({ create, read }) => create && read).forEach(access => {
        it.skip(`does not show create option when no fields are creatable (list view): ${JSON.stringify(access)}`, () => {
          // NOTE: This is different from the test in List where it is testing
          // the List-level permissions. Here, we are testing the field-level
          // permissions
          // TODO
        ...
        it.skip(`does not show create option when no fields are creatable (item view): ${JSON.stringify(access)}`, () => {
          const name = getDynamicListName(access);
          const queryName = `all${name}s`;
          const slug = listSlug(name);
  ...
  describe('updating', () => {
    describe('static', () => {
      ...
      accessCombinations.filter(({ update, read }) => update && read).forEach(access => {
        it(`shows updatable fields as inputs: ${JSON.stringify(access)}`, () => {
          const name = getStaticListName(access);
          const queryName = `all${name}s`;
          const slug = listSlug(name);
      ...
      accessCombinations.filter(({ update, read }) => !update && read).forEach(access => {
        it.skip(`does not show input fields when not updatable: ${JSON.stringify(access)}`, () => {
          const name = getStaticListName(access);
          const queryName = `all${name}s`;
          const slug = listSlug(name);
    ...
    describe('dynamic', () => {
      ...
      // non-readable lists show access denied above
      accessCombinations.filter(({ create, read }) => create && read).forEach(access => {
        it(`shows update option when updatable: ${JSON.stringify(access)}`, () => {
